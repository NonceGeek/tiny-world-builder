...
- When testing a planet/ground surface below floating islands, keep it as a separate lowered LandscapeEngine instance (`planetLandscapeEngine`) instead of flipping `useLandscapeEngine`/`landscapeMeshMode`. That preserves the editable floating board and ghost-board behavior while the underlay streams independently. Keep near detail modest but give the far LOD enough coverage to fade out in every direction (currently near 2 / far 6, with reduced far chunk resolution), with underlay-specific fog uniforms so the finite chunk edge disappears into the horizon. Planet distance is user-adjustable through the Generate modal / query `planetDrop`; changing it should move the lowered LandscapeEngine group and rescale the between-layer atmosphere sheets, not alter the floating board height. Add the island-to-planet atmosphere as a few cheap transparent world-space haze sheets between the board and underlay (`planetAtmosphereGroup`), not as a global post blur; depth testing keeps the editable island crisp while softening only the lower landscape behind it.
